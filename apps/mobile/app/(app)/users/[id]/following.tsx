...
//   · Top bar: back + Kicker count (meta.total) + Heading "Siguiendo"
//   · Body: card-shelled FlatList, paginated (GET /users/:id/following?page&limit)
...
export default function Following() {
  ...
  const fetchPage = useCallback(
    async (page: number) => {
      ...
      const r = await usersApi.following(id, { page, limit: PAGE_SIZE });
  ...
  // Follow/unfollow events performed by this user → silent refresh.
  useRealtime('user', (env) => {
    const d: any = env?.data ?? {};
    if (typeof d?.follow !== 'boolean') return;
    if (d?.by === id) void load(true);
  });
      ...
      {loading ? (
        ...
        isPrivateError ? (
          <EmptyState
            ...
            message={
              t
                ? 'Sigue a esta persona para ver a quién sigue.'
                : 'Follow this person to see who they follow.'
            }
        ...
        <EmptyState
          icon="user-plus"
          title={t ? 'Aún no sigue a nadie' : 'Not following anyone yet'}
          message={
            t
              ? 'Las personas a las que siga aparecerán aquí.'
              : 'People they follow will appear here.'
          }
        />
      ) : (
        <FlatList
      ...
      <View style={styles.rowText}>
        ...
        {u.since ? (
          <Caption tone="muted" numberOfLines={1} style={{ marginTop: 2 }}>
            {t ? 'Sigue desde' : 'Following since'} {formatSince(u.since, t)}
          </Caption>
        ) : null}
      </View>
